...
clf = RandomForestRegressor(n_estimators=1000, criterion='mae', bootstrap=True, oob_score=True, verbose=0.7,
							warm_start=True)
...
path1 = path_fig
fig.savefig(path1 + 'fig_model1.png')
plt.clf()
...
fin = df_ex3.to_json(path_file + 'Output_model1.json')

# Reading csv and saving new data there
rdf = pd.read_csv(path_file + 'Output_model1.csv')
...
rdf4 = rdf2.to_csv(path_file + 'Output_model1.csv', index=False)
...
fig.savefig(path_fig + 'fig_model1_compare.png')
plt.clf()

# Create output specification
text_file = open(path_file + "Model_specification_01.txt", "w")
